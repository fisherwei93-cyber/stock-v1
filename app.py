...
<style>
    ...
    .ec-sector {{ font-size: 10px; padding: 1px 4px; border-radius: 3px; background: #333; color: #aaa; margin-top: 4px;
    ...
    .ext-price {{ font-size: 16px !important; color: #9CA3AF;
    ...
    .l-item {{ display: flex; justify-content: space-between; align-items: center; font-size: 14px; font-weight: 600; border-bottom: 1px dashed rgba(0,0,0,0.2);
        padding: 3px 0; color: #000; }}
    
    /* 历史搜索 */
    .hist-tag {{
        display: inline-block;
        background: #333; color: #ccc; padding: 4px 10px; 
    ...
    /* 指标解释框 */
    .ind-desc {{ background: #111; border-left: 3px solid #3b82f6; padding: 8px;
        margin-top: 5px; font-size: 12px; color: #ccc; }}

    /* 通用组件 */
    .tg-s {{ background: rgba(0,0,0,0.1);
        padding: 1px 5px; border-radius: 4px; font-size: 11px; margin-left: 6px; color: #333; }}
    .earning-row {{ display: flex;
        justify-content: space-between; padding: 8px; border-bottom: 1px solid #333; font-size: 13px; }}
    .thesis-col {{ flex: 1; padding: 10px;
        border-radius: 6px; font-size: 13px; margin-top:5px; }}
    .thesis-bull {{ background: rgba(6, 78, 59, 0.8); border: 1px solid #34d399;
        color: #fff; }}
    .thesis-bear {{ background: rgba(127, 29, 29, 0.8); border: 1px solid #f87171; color: #fff; }}
    .score-card {{ background: #1A1A1A; padding: 15px; border-radius: 12px; text-align: center;
    ...
    .wl-row {{ background-color: #1A1A1A;
        padding: 12px; margin-bottom: 8px; border-radius: 6px; border-left: 4px solid #555; display: flex; justify-content: space-between; align-items: center; color: #FFFFFF; }}
    .social-box {{ display: flex; gap: 10px; margin-top: 10px; }}
    .mc-box {{ background: #0f172a;
        border: 1px solid #1e293b; padding: 10px; border-radius: 6px; margin-top:5px; }}
    .note-box {{ background: #1e1b4b;
        border-left: 4px solid #6366f1; padding: 10px; font-size: 12px; color: #e0e7ff; margin-top: 5px; border-radius: 4px; line-height: 1.6; }}
    ...
    .report-title {{ font-size: 22px; font-weight: 900;
        color: #FF9F1C; margin-bottom: 10px; border-left: 5px solid #FF9F1C; padding-left: 10px; }}
    .report-text {{ font-size: 15px; line-height: 1.8;
        color: #E5E7EB; margin-bottom: 20px; background: #1A1A1A; padding: 15px; border-radius: 8px; }}
    .guru-check {{ display: flex; align-items: center;
        margin-bottom: 8px; padding: 8px; background: #262626; border-radius: 6px; }}
    ...
    .hold-name {{ font-weight: 600;
        font-size: 13px; color: #f3f4f6; letter-spacing: 0.5px; }}
    ...
    .wiki-card {{ background: #1A1A1A; border: 1px solid #333; border-radius: 8px; padding: 20px;
        margin-bottom: 20px; }}
    .wiki-title {{ font-size: 20px; font-weight: bold; color: #FF9F1C; margin-bottom: 15px;
...
# [FIX] V106 核心修复: 恢复 V104 的全量数据获取逻辑，并增加 NVDA 防崩熔断
...
def fetch_financial_data_v106(ticker):
    ...
    # [FIX] V106 关键修改: 对每个易崩模块单独做 Try-Except，防止 NVDA 搜索崩溃
    # 并且如果 info 拿不到，尝试用 fast_info 补救
    safe_info = {}
    try:
        safe_info = s.info
    ...
    # 补救措施: 如果 info 为空 (NVDA 常见情况)，尝试用 fast_info 填充关键字段
    if not safe_info or 'marketCap' not in safe_info:
        try:
            ...
            safe_info['dividendYield'] = 0 # 无法获取时默认为0
        except: pass
    ...
    return {"history": h, "info": safe_info, "compare": cmp_norm, "error": None, "upgrades": upgrades, "inst": inst, "insider": insider, "fin": fin, "options": None}
...
def calculate_vision_analysis(df, info):
    ...
    ma20 = df['MA20'].iloc[-1];
    ma60 = df['MA60'].iloc[-1]; ma200 = df['MA200'].iloc[-1]
    ...
    val_data = f"{eps_fwd*25:.0f}-{eps_fwd*35:.0f}" if eps_fwd else "N/A"
...
def generate_bull_bear_thesis(df, info):
    ...
    bulls = [];
    bears = []
    curr = df['Close'].iloc[-1]; ma200 = df['MA200'].iloc[-1];
    rsi = df['RSI'].iloc[-1]
...
def calculate_quant_score(info, history):
    ...
    if pe and pe < 30: score += 10;
    notes.append("估值健康")
    gr = info.get('revenueGrowth')
    if gr and gr > 0.15: score += 10;
    notes.append("高成长")
    rec = info.get('recommendationMean')
    if rec and rec < 2.0: score += 15;
    notes.append("机构强推")
    ...
    if not h.empty:
        ...
        if l_an:
            ...
            <div class='l-box'>
                ...
                <div class='l-item'><span>技术面诊断</span><span style='font-weight:bold;
color:#2563EB'>{l_an['tech']}</span></div>
        ...
        with st.expander("📉 进阶指标 (Z-Score/ADX/CCI)", expanded=False):
            ...
            fig_c.add_hline(y=100, line_dash='dot', line_color='red');
            fig_c.add_hline(y=-100, line_dash='dot', line_color='green')
        ...
        with c2:
            ...
            if heavy.get('insider') is not None and not heavy['insider'].empty:
                for index, row in heavy['insider'].head(10).iterrows():
                    ...
                    action = "❓ 未知";
                    color = "#9ca3af"
                    if "Sale" in trans or "Sold" in trans: action = "🔴 减持";
                    color = "#ef4444"
                    elif "Purchase" in trans or "Buy" in trans: action = "🟢 增持";
                    color = "#4ade80"
                    ...
                    <div class='hold-card'>
                        <div>
                            <div class='hold-name'>{insider}</div>
                            <div class='hold-sub'>{relation}</div>
                        </div>
                        <div style='text-align:right'><div style='color:{color};font-weight:bold'>{action} (均价 {price})</div><div class='hold-val'>{shares}股</div></div>
                    </div>
                    """, unsafe_allow_html=True)
            else: st.info("暂无数据")
    ...
    with tabs[3]:
        ...
        # [RECOVERED] Full Guru Checklist (Missing in previous context)
    ...
    st.markdown("""
    <div class='wiki-card'><div class='wiki-title'>1.
    视野·交易计划 (Vision L-Box)</div><div class='wiki-text'><b>核心逻辑：</b> L战法系统。<br><b>黄框</b>：系统大脑。<br><span class='wiki-tag'>R1/R2</span> 压力位。<br><span class='wiki-tag'>S1/S2</span> 支撑位。</div></div>
    <div class='wiki-card'><div class='wiki-title'>2.
    神奇九转 (TD Sequential)</div><div class='wiki-text'><b>原理：</b> 寻找衰竭点。<br><span style='color:#f87171'><b>红色 9</b></span>：上涨力竭(卖)。<br><span style='color:#4ade80'><b>绿色 9</b></span>：下跌力竭(买)。</div></div>
    <div class='wiki-card'><div class='wiki-title'>3.
    VWAP (机构线)</div><div class='wiki-text'><b>原理：</b> 机构持仓成本。<br>股价 > VWAP：机构护盘。<br>股价 < VWAP：机构出货。</div></div>
    <div class='wiki-card'><div class='wiki-title'>4.
    蒙特卡洛预测 (Monte Carlo)</div><div class='wiki-text'><b>原理：</b> 模拟未来30天100种走势。<br><b>悲观底线</b>：95%概率不跌破的止损位。</div></div>
    <div class='wiki-card'><div class='wiki-title'>5.
    六维雷达 (Spider)</div><div class='wiki-text'><b>原理：</b> 公司体检表。面积越大，基本面越完美。</div></div>
    <div class='wiki-card'><div class='wiki-title'>6. SuperTrend</div><div class='wiki-text'><b>原理：</b> 趋势跟踪。<b>绿色</b>持有，<b>红色</b>空仓。</div></div>
    <div class='wiki-card'><div class='wiki-title'>7.
    FVG (缺口)</div><div class='wiki-text'><b>原理：</b> 机构暴力拉升留下的<b>紫色方块</b>。股价常会回调填补。</div></div>
    <div class='wiki-card'><div class='wiki-title'>8.
    Z-Score (乖离)</div><div class='wiki-text'><b>原理：</b> 统计学偏差。<br>>2: 涨过头(回调风险) <br><-2: 跌过头(反弹机会)。</div></div>
    <div class='wiki-card'><div class='wiki-title'>9.
    唐奇安通道</div><div class='wiki-text'><b>原理：</b> 海龟交易法。<br>突破上轨买，跌破下轨卖。</div></div>
    <div class='wiki-card'><div class='wiki-title'>10. Ichimoku (一目均衡)</div><div class='wiki-text'><b>原理：</b> 云带系统。<br>股价在云上为多，云下为空。</div></div>
    <div class='wiki-card'><div class='wiki-title'>11.
    ADX (趋势强度)</div><div class='wiki-text'><b>原理：</b> 判断有无趋势。<br>>25: 趋势强劲。<br><20: 震荡市(休息)。</div></div>
    <div class='wiki-card'><div class='wiki-title'>12.
    HMA (赫尔均线)</div><div class='wiki-text'><b>原理：</b> 零滞后均线，比MA更快。</div></div>
    <div class='wiki-card'><div class='wiki-title'>13. 凯利公式</div><div class='wiki-text'><b>原理：</b> 科学仓位管理。告诉你这把牌该下注多少钱。</div></div>
    <div class='wiki-card'><div class='wiki-title'>14.
    CCI (顺势指标)</div><div class='wiki-text'><b>原理：</b> 抓极端行情。<br>>100: 超买。<br><-100: 超卖。</div></div>
    """, unsafe_allow_html=True)