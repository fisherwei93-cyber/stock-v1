...
if page == "🚀 股票分析":
    ...
    with tabs[1]:
        ...
        with c1:
            ...
            if heavy.get('inst') is not None:
                ...
                st.caption("💡 点击机构名可跳转 WhaleWisdom 查看调仓。")
        ...
        with c2:
            ...
            if heavy.get('insider') is not None:
                for index, row in heavy['insider'].head(10).iterrows():
                    ...
                    st.markdown(f"<div class='hold-card'><div class='hold-link'><a href='{link}' target='_blank'>{row.get('Insider')}</a><div class='hold-sub'>{row.get('Position')}</div></div><div style='text-align:right'><div style='color:{'#ef4444' if 'Sale' in trans else '#4ade80'};font-weight:bold'>{act} ({price})</div><div class='hold-val'>{row.get('Shares')}股</div></div></div>", unsafe_allow_html=True)
    ...
    with tabs[2]:
        ...
        else: st.metric("Graham Number", "N/A", "数据不足")
